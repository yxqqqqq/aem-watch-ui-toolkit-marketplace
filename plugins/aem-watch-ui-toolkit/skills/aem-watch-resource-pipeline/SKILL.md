...
description: "按项目 JSON 配置处理 AEM Watch 的正式图片、旧版 XLS 翻译表、bt_watch.ui、UI Editor 事务化生成和资源状态检查。涉及图片或字符串资源、新 board/resolution 接入或正式资源生成时使用；普通 C/C++ 布局和非资源缺陷修复不使用。"
---

# AEM Watch 正式资源流水线

本 Skill 是一次性的资源执行工作流：接收明确的操作和项目 JSON，校验环境，调用翻译表后端或 UI Editor，执行 Post 后把结果交还主任务。它不重新搜索或解释 `AGENTS.md`；当前任务已经加载的用户要求和项目规则负责授权、选择 profile 及决定单次交付或批量收尾。
...
| 路径 | 何时使用 | 环境门槛 |
|---|---|---|
| 图片/UI 资源 | 图片、资源 ID、`bt_watch.ui`、UI Editor 输出或资源状态变化 | `ResourceEnvironment`：项目文件、UI Editor、Python |
| 文本翻译 | 新增或修改字符串 key、翻译表或打包语言文本 | `TranslationEnvironment`：翻译表与实际选中的 POI/WPS 后端 |

只改 C/C++ 布局且正式资源输入没有变化时退出本 Skill。文本后端不可用只阻塞文本路径，不阻塞纯图片/UI 资源检查和生成。默认 `auto` 在标准 OLE/BIFF8 工作簿上使用内置 POI 写入器；配置了 `protectedWrapperTokens` 的工作簿使用 WPS。不得使用未验证的普通 XLS 库直接覆盖正式表。
...
- 翻译表写入只通过 `TranslationApply`：先备份，再由选定后端写临时文件并重新验证；不是自由表格编辑流程。
...
- `TranslationApply`：备份后由选定后端写入并重新验证；POI 路径保持非 `Workbook` OLE 流和根 CLSID 不变，WPS 路径保持保护包装。
...
文本、完整资源或新档案任务读取 [final-report.md](references/final-report.md)。只报告实际载入的配置、选中的文本后端、修改的正式输入、WPS/UI Editor 是否运行、实际输出和已执行检查；未执行的生成、编译、Figma、视觉或真机验证必须明确说明。
