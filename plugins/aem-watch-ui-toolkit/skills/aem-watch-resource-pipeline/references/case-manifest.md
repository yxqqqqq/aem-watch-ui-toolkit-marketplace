...
- `active_languages` 可省略；流水线会从翻译表语言代码行读取实际启用语言。若填写，它只作为安全断言，必须与工作簿完全一致。不得从示例或 `translation.languageColumns` 推断当前启用语言。
- 每个新增字符串覆盖流水线检测到的全部启用语言。AI 生成的翻译在完成报告中标记待人工审核；不熟悉且未获授权的语言不要自行填充。
